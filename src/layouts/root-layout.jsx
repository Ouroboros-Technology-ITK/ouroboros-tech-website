...
const NavLink = forwardRef(({ children, href, ...props }, forwadedRef) => (
  <Link
    ...
    ref={forwadedRef}
    href={href}
    className="group h-max w-max overflow-hidden rounded-md text-2xl font-medium text-brand-secondary-00/90 outline-none duration-200 hover:text-brand-secondary-00/100 focus:outline-none focus:ring-1 focus:ring-brand-secondary-00/60 active:brightness-90 sm:text-sm md:text-base xl:text-lg"
  >
    <div className="before:content-[' '] relative z-50 w-max px-1 before:absolute before:bottom-0 before:left-0 before:top-1/2 before:w-5/12 before:rounded-sm before:bg-brand-trinity-06/30 before:transition-all before:duration-200 hover:before:w-full group-hover:before:bg-brand-trinity-06/100 group-focus:before:w-full group-data-[state=on]:before:w-full group-data-[state=on]:before:bg-brand-trinity-06/100">
...
export const RootLayout = ({ children }) => {
  ...
  return (
    <>
      <header className="sticky top-0 flex justify-center bg-brand-secondary-06/90 p-4 backdrop-blur-md">
        <div className="flex w-full items-center justify-between sm:container">
          ...
          <nav className="flex items-center">
            ...
            <ToggleGroupRadix.Root type="single" value={currentPage} asChild>
              <ul
                className={cn(
                  "absolute top-full left-0 right-0 bottom-0 h-screen w-full flex-col gap-8 bg-brand-secondary-06 py-8 px-2 pl-8 sm:static sm:flex sm:h-max sm:flex-row sm:gap-3 sm:px-0 sm:py-0 md:gap-4 xl:gap-6 2xl:gap-8",
                  {
                    hidden: !menuIsOpen,
                    flex: menuIsOpen,
                  },
      ...
      <footer className="h-max bg-brand-secondary-06 p-4">
