...
>(({ className, ...props }, ref) => {
  ...
  return (
    <NavigationMenuPrimitive.Link
      ...
      href={props.href}
      className={cn(
        "group mx-1 inline-flex h-10 w-max items-center justify-center rounded-md px-4 py-2 text-sm font-bold text-brand-secondary-00/60 transition-colors hover:bg-stone-100 hover:text-brand-trinity-04 focus:bg-stone-100 focus:text-brand-trinity-04 focus:outline-none disabled:pointer-events-none disabled:opacity-50 data-[active]:bg-stone-100/50 data-[state=open]:bg-stone-100/50 dark:bg-stone-950 dark:hover:bg-stone-800 dark:hover:text-stone-50 dark:focus:bg-stone-800 dark:focus:text-stone-50 dark:data-[active]:bg-stone-800/50 dark:data-[state=open]:bg-stone-800/50 lg:text-base",
        {
          "rounded-b-none border-b-2 border-b-brand-trinity-04 font-bold text-brand-trinity-04":
            isActive, // Add bg-white class if the link is active
        },
        className, // Keep any additional classes passed from the parent component
      )}
      {...props}
    />
  )
